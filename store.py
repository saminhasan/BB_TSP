    ...
    def populate_shelves(self):
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                ...
                products = [Product(f"Product{i*self.cols+j+k}", random.randint(100000000, 999999999), random.randint(10000, 99999), position) for k in range(25)]
                self.shelves[i][j] = Shelf(position, cx, cy, random_length, random_width, products)
